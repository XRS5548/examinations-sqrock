...
export default function InternshipCertificatePDF({
  ...
  const description =
    ...
    `This is to certify that ${data.internName} has successfully completed an internship with ${data.companyName} as a ${data.designation}${
      data.department
        ? ` in the ${data.department} department`
        : ""
    } from ${data.startDate} to ${data.endDate}${
      data.duration ? ` for a duration of ${data.duration}` : ""
